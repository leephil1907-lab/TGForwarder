...
export class TelegramEngine {
  ...
  private constructor() {
    ...
    this.initStats();
    // Keep the Telegram connection alive for the life of the process. GramJS
    // auto-reconnect handles most drops; this watchdog catches the rest so the
    // account only goes offline when the user explicitly disconnects.
    this.connectionWatchdog = setInterval(() => {
      try {
        const client: any = this.client;
        if (!client || this.authState.status !== 'connected') return;
        const isConnected = typeof client.isConnected === 'function' ? client.isConnected() : Boolean(client.connected);
        if (isConnected || this.reconnecting) return;
        this.reconnecting = true;
        Promise.resolve(client.connect())
          .then(() => this.log({ level: 'info', category: 'auth', title: '🔗 Connection Restored', message: 'The Telegram connection dropped and was restored automatically. Your session stays logged in.' }))
    ...
    if (typeof this.connectionWatchdog.unref === 'function') this.connectionWatchdog.unref();
  }
  ...
  /** Direct access to the connected MTProto client (used by the Restricted Fetcher worker). */
  public getClient(): TelegramClient | null {
    return this.client;
  }

  public isAccountConnected(): boolean {
    return this.authState.status === 'connected' && Boolean(this.client);
  }

  /** Broadcasts a custom event to all connected SSE dashboard clients. */
  public broadcastEvent(type: string, payload: any): void {
  ...
  public initializeFromStorage(): Promise<boolean> {
    // Only one initialization attempt per process. A second concurrent call
    // would create a second MTProto client on the same session, which Telegram
    // punishes with migration errors and random disconnects.
  ...
  private async dispatchItem(item: QueuedMessage, rateLimit: RateLimitConfig) {
    ...
    try {
      ...
      try {
        ...
        // ================= ENGINEERING FALLBACK (save-restricted-bot method) =================
