      ...
      if (routePath === '/api/history' && handlers.length) {
        ...
        originalGet.call(this, '/api/history/media', async (req: any, res: Response) => {
          try {
            ...
            res.setHeader('Content-Type', mime); res.setHeader('Content-Length', String(body.length)); return res.end(body);
          } catch (error: any) {
        ...
        originalGet.call(this, '/api/history/media/thumbnail', async (req: any, res: Response) => {
          try {
            ...
            if (!sourceId || !Number.isFinite(messageId) || messageId <= 0) return res.status(400).json({ error: 'sourceId and messageId are required.' });
            const { client, message, meta } = await loadTelegramMessage(sourceId, messageId);
            // Ask Telegram/GramJS for the smallest available thumbnail. This avoids
            // downloading an entire video merely to paint a dashboard preview.
            const thumb = await client.downloadMedia(message, { thumb: -1, workers: 1 });
            if (!thumb) return res.status(404).json({ error: 'Telegram thumbnail is not available.' });
            const body = Buffer.isBuffer(thumb) ? thumb : Buffer.from(thumb as any);
