...
app.get('/api/health', async (_req, res) => {
  try {
    ...
    const auth = engine.getAuthState();
    res.json({ status: 'ok', worker: { status: auth.status === 'connected' ? 'online' : 'offline', engineRunning: engine.isEngineRunning(), isPaused: engine.isEnginePaused() }, engineRunning: engine.isEngineRunning(), isPaused: engine.isEnginePaused(), authStatus: auth.status, authenticated: auth.status === 'connected', userProfile: auth.userProfile, timestamp: Date.now() });
