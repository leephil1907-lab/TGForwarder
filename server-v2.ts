...
async function startServer() {
  ...
  app.post('/api/config', (req, res) => {
    ...
    storage.saveConfig({ ...(apiId !== undefined && { apiId }), ...(apiHash !== undefined && { apiHash }), ...(defaultRemoveSignature !== undefined && { defaultRemoveSignature }), ...(retryOnFloodWait !== undefined && { retryOnFloodWait }), ...(globalRateLimit !== undefined && { globalRateLimit }), ...(accounts !== undefined && { accounts }) });
  ...
  app.post('/api/auth/request-code', async (req, res) => {
    try { clearEngineLogs(engine); const { apiId, apiHash, phoneNumber } = req.body; if (!apiId || !apiHash || !phoneNumber) return res.status(400).json({ error: 'API ID, API Hash, and Phone Number are required.' }); res.json(await engine.requestPhoneCode(apiId, apiHash, phoneNumber)); }
    catch (err: any) { res.status(500).json({ error: err.message || 'Failed to request verification code' }); }
  });
  app.post('/api/auth/verify-code', async (req, res) => {
    try { const { phoneCode } = req.body; if (!phoneCode) return res.status(400).json({ error: 'Phone verification code is required.' }); res.json(await engine.verifyCode(phoneCode)); }
    catch (err: any) { res.status(500).json({ error: err.message || 'Failed to verify code' }); }
  });
  app.post('/api/auth/verify-2fa', async (req, res) => {
    try { const { password } = req.body; if (!password) return res.status(400).json({ error: '2FA password is required.' }); res.json(await engine.verify2FA(password)); }
    catch (err: any) { res.status(500).json({ error: err.message || '2FA verification failed' }); }
  });
  app.post('/api/auth/bot-login', async (req, res) => {
    try { clearEngineLogs(engine); const { apiId, apiHash, botToken } = req.body; if (!apiId || !apiHash || !botToken) return res.status(400).json({ error: 'API ID, API Hash, and Bot Token are required.' }); res.json(await engine.connectWithBotToken(apiId, apiHash, botToken)); }
    catch (err: any) { res.status(500).json({ error: err.message || 'Failed to connect bot' }); }
  });
  app.post('/api/auth/session-login', async (req, res) => {
    try { clearEngineLogs(engine); const { apiId, apiHash, sessionString } = req.body; if (!apiId || !apiHash || !sessionString) return res.status(400).json({ error: 'API ID, API Hash, and Session String are required.' }); res.json(await engine.connectWithStringSession(apiId, apiHash, sessionString)); }
    catch (err: any) { res.status(500).json({ error: err.message || 'Failed to connect session' }); }
  });
  app.post('/api/auth/disconnect', async (_req, res) => {
    try { await engine.disconnect(); clearEngineLogs(engine); res.json({ success: true, message: 'Disconnected successfully. Session activity was cleared.' }); }
    catch (err: any) { clearEngineLogs(engine); res.status(500).json({ error: err.message || 'Error disconnecting' }); }
  });
  ...
  app.get('/api/stream', (req, res) => {
    ...
    let closed = false; const write = (chunk: string) => { if (closed || res.writableEnded) return; try { res.write(chunk); } catch { closed = true; } };
    ...
    req.on('close', cleanup); req.on('end', cleanup); res.on('finish', cleanup); res.on('error', cleanup);
  });
