...
// Ensure the engine-start console event carries the actual configured IDs.
...
if (!engineProto.__tgforwarderManualReview) {
  ...
  engineProto.dispatchItem = async function (item: any, rateLimit: any) {
    ...
    this.log({ level: 'info', category: 'forward', title: '📝 REVIEW: Incoming Post Awaiting Approval', message: `Post #${message?.id} is waiting for edit/preview/publish approval.`, sourceId: chatId, sourceTitle: item.rule?.sourceTitle || chatId, targetId: item.targetId, targetTitle: item.targetTitle || item.targetId, messageSnippet: (item.processedText || '').slice(0, 80) });
  };
