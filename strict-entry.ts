...
// Telegram/GramJS dialog discovery is lazy. Use iterDialogs so the account is
// not artificially capped at the previous 150-dialog getDialogs request.
if (!engineProto.__tgforwarderAllDialogs) {
  ...
  engineProto.discoverChats = async function () {
    ...
    this.log({ level: 'info', category: 'discovery', title: 'Discovering All Telegram Dialogs', message: 'Loading channels, groups, bots, users and chats with Telegram pagination.' });
    ...
    this.log({ level: 'success', category: 'discovery', title: 'Chat Discovery Complete', message: `Found ${discovered.length} Telegram entities across all available dialog pages.` });
    return discovered;
  };

  // If an exact numeric channel is outside the first 150 dialogs, warm the
  // entity cache from all available dialogs before the final getEntity call.
...
// Ensure every log that has an ID displays that ID even when a human-readable
// title is unavailable. IDs remain authoritative for routing.
    ...
    if (logItem?.title === 'Forwarding Engine Active' && !logItem.sourceId) {
      ...
      if (active) {
        logItem = {
          ...
          sourceTitle: active.sourceTitle || active.sourceId,
          targetId: active.targetIds[0],
          targetTitle: active.targetTitles?.[0] || active.targetIds[0],
          message: `${logItem.message} Source ${active.sourceId} → Target ${active.targetIds[0]}.`
        };
      }
    }
...
// New incoming posts are held for operator review before final dispatch.
if (!engineProto.__tgforwarderManualReview) {
  ...
  engineProto.dispatchItem = async function (item: any, rateLimit: any) {
    ...
    this.log({
      level: 'info',
      category: 'forward',
      title: '📝 REVIEW: Incoming Post Awaiting Approval',
      message: `Post #${message?.id} is waiting for edit/preview/publish approval.`,
      sourceId: chatId,
      sourceTitle: item.rule?.sourceTitle || chatId,
      targetId: item.targetId,
      targetTitle: item.targetTitle || item.targetId,
      messageSnippet: (item.processedText || '').slice(0, 80)
    });
  };

  engineProto.getPendingPosts = function () {
    return Array.from(pending.values()).map(({ item, ...post }) => post);
  };
  ...
  engineProto.publishPendingPost = async function (key: string, text?: string) {
    ...
    try {
      ...
      // originalDispatch handles Telegram errors internally and does not throw;
      // verify success through the mapping count is not reliable, so publish only
      // reports success after the dispatch path has completed. Its own telemetry
      // records the exact Telegram failure if the target rejects the send.
      pending.delete(key);
  ...
  engineProto.discardPendingPost = function (key: string) {
    return { success: pending.delete(key) };
  };

  const originalDisconnect = engineProto.disconnect;
  engineProto.disconnect = async function () {
    pending.clear();
...
// History/manual publishing routes in server-v2 call TelegramClient methods
// directly. Resolve exact numeric chat IDs first so a target outside the first
// 150 dialogs still publishes correctly and so private-channel access hashes
// are respected by GramJS.
  ...
  if (!clientProto.__tgforwarderClientSendResolution) {
    ...
    clientProto.sendMessage = async function (entityOrPeer: any, ...args: any[]) {
      ...
      if (typeof entityOrPeer === 'string' || typeof entityOrPeer === 'number') {
        try { resolved = await this.getEntity(entityOrPeer); } catch (error: any) {
          throw new Error(`Unable to resolve Telegram destination ${String(entityOrPeer)}: ${error?.message || 'entity not accessible'}`);
        }
      }
