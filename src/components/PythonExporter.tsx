...
API_ID = ${Number(config?.apiId) > 0 ? Number(config?.apiId) : 0}  # TODO: your numeric API ID from https://my.telegram.org
API_HASH = ""  # TODO: paste your API hash here (the dashboard masks it; copy it from your original credentials)

# Channel Configuration (IDs or @usernames)
SOURCE_CHANNELS = ${JSON.stringify(sourceChannels, null, 4)}${sourceChannels.length ? '' : '  # TODO: add source channel IDs'}

DESTINATION_CHANNELS = ${JSON.stringify(destChannels, null, 4)}${destChannels.length ? '' : '  # TODO: add destination channel IDs'}
...
AUTO_SLEEP_ON_FLOODWAIT = ${config?.globalRateLimit?.autoSleepOnFloodWait ? 'True' : 'False'}
