...
export default function App() {
  ...
  useEffect(()=>{
    ...
    const connect=()=>{if(!mounted)return; es?.close(); es=new EventSource(withTokenParam('/api/stream')); es.onmessage=(ev)=>{try{if(!ev.data||ev.data.startsWith(':'))return;const d=JSON.parse(ev.data);switch(d.type){case'INIT_SNAPSHOT':d.payload.authState&&setAuthState(d.payload.authState);d.payload.stats&&setStats(d.payload.stats);d.payload.recentLogs&&setLogs(d.payload.recentLogs);break;case'NEW_LOG':setLogs(v=>[...v.slice(-250),d.payload]);break;case'LOGS_CLEARED':setLogs([]);break;case'STATS_UPDATED':setStats(d.payload);break;case'AUTH_STATUS_CHANGED':setAuthState(d.payload);break;case'ENGINE_STATE_CHANGED':setConfig(v=>v?{...v,isEngineRunning:d.payload.isRunning}:v);break;case'PENDING_UPDATED':window.dispatchEvent(new CustomEvent('tgf-pending-updated',{detail:d.payload}));break;}}catch(e){console.error('SSE parse error',e);}}; es.onerror=()=>{es?.close();es=null;if(mounted){if(timer)clearTimeout(timer);timer=setTimeout(connect,2500);}};};
